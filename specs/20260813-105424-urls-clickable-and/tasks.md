...
- [X] T001 Record a green pre-change baseline: run `cd src-tauri && cargo test && cargo clippy -- -D warnings` and `pnpm test` from the repo root, and note that all three pass before any edit
- [X] T002 [P] Confirm `tauri-plugin-opener` is absent from `src-tauri/Cargo.toml`, `package.json`, and that `src-tauri/capabilities/default.json` needs no new permission entry — the first-party command route ([research.md](./research.md) §1) requires none
...
- [X] T009 Add `isOpenable` unit tests to `src/render.test.ts` mirroring the T004 case list (accepts both schemes and mixed case, rejects `ftp://`, `file://`, `javascript:`, bare `example.com`, and empty/whitespace)
...
- [X] T018 [P] [US2] Add reconciliation cases to `src/render.test.ts` in the existing style: the URL button's **element identity is preserved** across a `renderTable` call with only `now` advanced, and across one where a status event has landed (FR-011, US2 scenario 3)
- [X] T019 [P] [US2] Add a `src/render.test.ts` case asserting the URL button carries **no** `tabindex` attribute — it is in the tab order natively, and an explicit value would be a regression waiting to happen

### Implementation for User Story 2

- [X] T020 [US2] Add a `:focus-visible` outline for `.site-url` in `src/styles.css` — `:focus-visible` rather than `:focus`, so a mouse click does not leave a ring behind (FR-005)
...
- [X] T021 [P] [US3] Add a `src/render.test.ts` case: a site whose URL is not http/https renders as `<span class="site-primary">` with **no** `data-open-url`, no `site-url` class, and no `tabindex`. It is shown, not hidden and not flagged as invalid (FR-007, spec Open Decision 3).
- [X] T022 [P] [US3] Add a `src/open.test.ts` case: a rejected `openUrl` passes the backend's bare message to the error hook using the `String(message)` idiom `form.ts` already relies on, and the listener does not throw
- [X] T023 [US3] Add a `src/main.test.ts` case: an open failure reaches `showBanner`, and `renderTable` still runs afterwards — the table keeps updating and nothing else is disturbed (FR-009, US3 scenario 2, SC-006)

### Implementation for User Story 3

- [X] T024 [US3] Add the failure path to `mountUrlOpener` in `src/open.ts`: `catch` the rejected `openUrl` and route `String(message)` to the injected banner hook. It must go to the banner (`#banner` via `showBanner`) and **not** to the form's `#site-error` — the failure is not about anything the user typed. The command mutates nothing, so there is no partial state to unwind.
...
- [X] T025 Run the full merge bar from [quickstart.md](./quickstart.md#1-automated-the-merge-bar): `cd src-tauri && cargo test && cargo clippy -- -D warnings`, then `pnpm test` from the repo root. All three must be green. Confirm no test invokes the `open_url` command — `cargo test` must never launch a browser.
- [X] T026 [P] Confirm `git diff` touches **no** dependency manifest: not `src-tauri/Cargo.toml`, not `src-tauri/Cargo.lock`, not `package.json`, not `pnpm-lock.yaml`, and not `src-tauri/capabilities/default.json`. A change in any of them means the plugin route crept back in ([research.md](./research.md) §1).
- [~] T027 **Deferred — not run. Decided in this step; see [Decisions Made in This Step](#decisions-made-in-this-step) item 5 and surface it in the PR.** The manual pass needs `pnpm tauri dev`, and this run was unattended. Two reasons that combination is not safe to take unsupervised: the app has no config-directory override, so a dev run reads and writes the user's real `sites.json`; and more seriously, `lib.rs`'s startup calls `autostart::repair_if_stale`, which rewrites the user's `~/Library/LaunchAgents` entry to point at whatever copy is running — a throwaway worktree build, in this case — leaving launch-at-login aimed at a binary that will not survive this branch. Nobody was present to notice or undo either. What the pass would have proven that nothing else does: a real browser actually comes forward within two seconds (SC-003), the focus ring is visible (FR-005), and the tab order puts the URL ahead of Edit. **Run this before merging**, either by hand or via the optional `/speckit-qa-run` hook.
- [~] T028 **Partly done — the half that needs a running app is deferred with T027.** Verified without launching: `git status` shows no change to any dependency manifest or capability file (T026), and the whole existing suite still passes — `form.test.ts`, `main.test.ts`, and `store.rs`/`engine.rs`'s Rust tests cover adding, editing, deleting, status updates, and the age counters, all green. Not verified: `sites.json` byte-identical *after a real click*. That one needs the app running, which is what T027 defers; the command's own code path reads and writes no file and holds no lock, so there is no path by which it could write, but that is an argument rather than an observation.
- [X] T029 [P] Update the affected living specs via `/speckit-companion-living-sync` — `capabilities/frontend/spec.md` gains the URL control, its keyboard path, and the banner surface; the backend capability gains `open_url` and `openable_url`. Do not hand-edit these files.
...
The three below were decided during **implementation**, in the same unattended conditions, and belong in the pull request alongside the four above.

5. **The manual pass (T027) was not run, and the app was never launched.** Reasoning in T027 itself. The short version: `pnpm tauri dev` would have rewritten the user's real launch-at-login entry to point at this worktree's build, via the `repair_if_stale` call in `lib.rs` startup, and would have read and written their real `sites.json` — both unsupervised, with no one to notice. The three things only that pass can prove (a browser actually comes forward within 2 s, the focus ring is visible, the URL precedes Edit in the tab order) are therefore **unproven**. Everything else in the feature is covered automatically. This is the one item that should not be waved through at review.

6. **`mountUrlOpener` takes an injectable clock.** `shouldOpen` is pure and needs none, but the listener tests have to prove that a double-click collapses to one open *and* that the address opens again after the window — which with a real `Date.now` would mean sleeping a second inside the suite. The seam is one optional hook, defaulted to `Date.now`, and production passes nothing. The alternative, faking timers around a delegated listener, is more machinery to get the same assertion.

7. **`.site-url` is declared above `.site-secondary` in `styles.css`, and that order is load-bearing.** A labelled row's URL carries both classes; they have equal specificity, so `.site-url`'s `font: inherit` would beat `.site-secondary`'s `font-size: 12px` if it came later, and the secondary line would jump to the primary line's size. There is no CSS test in this project to catch that, so the constraint is written as a comment at the rule. If that bothers a reviewer, the alternative is a compound `.site-secondary.site-url` selector duplicating the size and opacity — more specific but no longer a single source for those values.
