...
**39 of 47 complete, 2 obsolete, 2 half done, 4 genuinely open.** `v1.0.0` is published and
...
| | Tasks | |
|---|---|---|
| ✅ Complete | 39, incl. T006, T017, T043, T047 newly ticked | Credential set, formula audited on the runner, `v1.0.0` released, post-release verification green |
| ❌ Obsolete | T007, T008 | The six Apple secrets. No certificate and no App Store Connect key exist or are needed. |
| 🔶 Half done | T031, T044 | The machine halves passed on a runner; the lagging-tap edit and the human-on-a-second-Mac open |
...
- [~] T044 **VALIDATE — machine half done, human half open.** A clean `macos-latest` runner that has never built this project installs from the tap and passes every machine-checkable assertion (`e2e` in runs `31529397548` and `31529604271`, ~45s): the installed bundle carries **no** `com.apple.quarantine` — which is the premise of the whole formula design, now machine-checked rather than folklore — `codesign --verify --strict` passes, `codesign -dv` reports `Signature=adhoc`, the `bin` wrapper is on `PATH`, and `CFBundleShortVersionString` is `1.0.0`. **What is still unrun: a human on a second Mac.** Opening the app and confirming macOS shows no security dialog is OS behaviour, not Homebrew's, and no runner can assert it; nor has the `/Applications` symlink been tried, nor SC-001's under-5-minutes timed. — quickstart Scenario 6, on a Mac (or fresh user account) that has never built this project: `brew install clintcparker/tap/site-checker`, then ~~`spctl -a -t exec -vv`~~ **`codesign --verify --strict --verbose=2`** on `"$(brew --prefix site-checker)/libexec/Site Checker.app"` *(`spctl` cannot pass on an ad-hoc signature — that is the recorded, accepted loss)*, then `CFBundleShortVersionString` via `PlistBuddy` (expect `1.0.0`). Open the app and confirm no security dialog and no right-click-open (SC-002). **Time the whole thing** — SC-001 claims under 5 minutes. Paste the `codesign` output into the ship record; it is the artifact worth keeping.
