...
feature's test suite. The standing gates (~~`cargo test` 55, `pnpm test` 30~~ → **62 and 47**
since the correctness and coverage work of 2026-08-11; `cargo clippy -- -D warnings`) must stay
green throughout — no application code is touched.
...
## Status — 2026-08-11 (supersedes the 2026-08-06 block below)

**40 of 47 complete, 2 obsolete, 1 half done, 4 genuinely open.** `v1.0.0` is published and
`brew install clintcparker/tap/site-checker` works. Standing gates: `cargo test` **62**,
`pnpm test` **47**, `cargo clippy -- -D warnings` clean, `pnpm build` clean, `actionlint` clean.

Every marker below was set against **named evidence** — a run id, a `gh` query, or a commit —
because this feature's records have twice carried a "done" that was aspirational. Where the
evidence did not exist, the task stayed open and says so.

| | Tasks | |
|---|---|---|
| ✅ Complete | 40, incl. T006, T017, T043, T047 and now **T044** newly ticked | Credential set, formula audited on the runner, `v1.0.0` released, post-release verification green, and **a human has installed it and watched it open with no dialog** |
| ❌ Obsolete | T007, T008 | The six Apple secrets. No certificate and no App Store Connect key exist or are needed. |
| 🔶 Half done | T031 | The machine half passed on a runner; the lagging-tap edit is still open |
| ⬜ Open | T027, T035, T045, T046 | **All four are failure paths and re-run paths that have never once executed** |

**The four open tasks share one shape, and it is worth naming.** T027 (pre-flight fails on a
missing secret), T035 (the CI version sentinel fires), T045 (uninstall keeps the site list), and
T046 (a re-run converges) are each a guard or a recovery path. Five green release runs and 15
green CI runs prove the *happy* path thoroughly and prove none of these at all: no release run has
ever failed, no CI run has ever failed, nothing has been re-run, and no uninstall has happened.
They are plausible-looking YAML and plausible-looking reasoning until something exercises them.
Filed as issues so they outlive this document.
...
- [X] T006 [P] Create a fine-grained personal access token with **Contents: Read and write** on **`clintcparker/homebrew-tap` only** (FR-015, research R15) and set it with `gh secret set TAP_PUSH_TOKEN --repo clintcparker/site-checker`. Verify the scope by listing the token's repository access — it must name exactly one repository. *(Done 2026-08-11 18:41 UTC. `gh secret list` shows `TAP_PUSH_TOKEN` and nothing else — the whole Apple set is gone. Five `homebrew` job runs have pushed to the tap with it. The token's own scope is not machine-checkable from this repository; it was set by hand to the single tap.)*
...
- [X] T017 [US1] **VALIDATE** *(amended and relocated — see the reconciliation block)* — hand-render `install/homebrew/site-checker.rb` to a scratch path with a plausible version and two 64-hex checksums, then run `ruby -c` on it (expect `Syntax OK`) and ~~`brew audit --cask --new`~~ **`brew audit --formula --new`** against it. Confirm zero placeholder tokens survive ~~and the `zap` path is the literal `~/Library/Application Support/com.clintparker.site-checker`~~ *(no `zap` stanza exists on a formula)*. *(Done 2026-08-11. `ruby -c` now runs inside `release.yml`'s render step, so it gates every release rather than being a one-time check. `brew audit --formula --new` runs non-gating on the macOS runner in `verify-install-channels.yml` — **never on the maintainer's machine** — and earned its place on its first run: 6 problems, 4 of them the same real defect (`on_arm` cannot carry `url`), fixed in PR #12. One finding is declined on record — see the open issue.)*
...
- [ ] T027 [US3] **VALIDATE — still open, and now known to be untested rather than assumed.** No `release.yml` run has ever failed: all five (four throwaway `v0.0.1` rounds and `v1.0.0`) are green, and `TAP_PUSH_TOKEN` was set at 18:41 UTC, two minutes before the first one. So neither the missing-secret path nor the malformed-tag path has executed once. — quickstart Scenario 5 (FR-017, FR-018, SC-005): with the repository public and secrets set, `gh secret delete TAP_PUSH_TOKEN` temporarily, push the throwaway tag `v0.0.1`, and `gh run watch`. Expect failure in under 60 seconds naming `TAP_PUSH_TOKEN` and pointing at `docs/how-to/release.md`, with no release object created and the tap unchanged. Then delete the tag, delete any release it produced, and restore the secret. Separately push `v0.0.1-preflight-test` to confirm the version-validation gate rejects it before the secret probe matters.
...
- [~] T031 [US4] **VALIDATE — first half done, second half open.** `gh workflow run verify-install-channels.yml` against the published `v1.0.0` is green (run `31529604271`, both `verify` and `e2e`). The lagging-channel half — hand-edit the tap's `version` to something older and expect a failure naming the tap — has **not** been run, and neither has R001's new checksum-drift check (leave the version correct, corrupt a `sha256`; the old workflow passed that case). — quickstart Scenario 8 (FR-019, FR-020, SC-006): `gh workflow run verify-install-channels.yml` and expect green against the current release. Then hand-edit `Casks/site-checker.rb` in the tap to an older `version` and re-dispatch; expect a failure naming the tap as the lagging channel. Restore by re-running `release.yml`'s `homebrew` job — which also proves the tap is machine-recoverable and never needs hand-editing.
...
- [X] T043 Merge the PR, then push the first real tag: `git tag -a v1.0.0 -m "site-checker 1.0.0"` and `git push origin v1.0.0`, then `gh run watch`. Expect a public release for `v1.0.0` carrying both `site-checker-aarch64-apple-darwin.zip` and `site-checker-x86_64-apple-darwin.zip` plus generated notes, and a tap commit `site-checker 1.0.0` creating ~~`Casks/`~~ **`Formula/`**`site-checker.rb`. *(Done 2026-08-11 19:44 UTC, run `31528419553`, annotated tag at `e0fe375`. Both assets published; the tap holds `Formula/site-checker.rb`. **Preceded by four throwaway `v0.0.1` rounds**, exactly as the plan intended, which found three real defects — Homebrew's stage-chdir, `.brew_home` inside the signed bundle, and `on_arm`/`on_intel` being an unsupported home for `url`/`sha256`. All torn down.)*
- [X] T044 **VALIDATE — both halves done.** A clean `macos-latest` runner that has never built this project installs from the tap and passes every machine-checkable assertion (`e2e` in runs `31529397548` and `31529604271`, ~45s): the installed bundle carries **no** `com.apple.quarantine` — which is the premise of the whole formula design, now machine-checked rather than folklore — `codesign --verify --strict` passes, `codesign -dv` reports `Signature=adhoc`, the `bin` wrapper is on `PATH`, and `CFBundleShortVersionString` is `1.0.0`. **The human half is now run as well** — opening the app and confirming macOS shows no security dialog is OS behaviour, not Homebrew's, and no runner can assert it, so a person did it; the `/Applications` symlink was tried and SC-001's under-5-minutes timed. See the note below. — quickstart Scenario 6, on a Mac (or fresh user account) that has never built this project: `brew install clintcparker/tap/site-checker`, then ~~`spctl -a -t exec -vv`~~ **`codesign --verify --strict --verbose=2`** on `"$(brew --prefix site-checker)/libexec/Site Checker.app"` *(`spctl` cannot pass on an ad-hoc signature — that is the recorded, accepted loss)*, then `CFBundleShortVersionString` via `PlistBuddy` (expect `1.0.0`). Open the app and confirm no security dialog and no right-click-open (SC-002). **Time the whole thing** — SC-001 claims under 5 minutes. Paste the `codesign` output into the ship record; it is the artifact worth keeping.
  > ✅ **Done 2026-08-11** — [qa/qa-20260811-135757.md](./qa/qa-20260811-135757.md) (in `qa/`, not `releases/`, because `specs/*/releases/` is gitignored and a record that is ignored is not kept), closing [#16](https://github.com/clintcparker/site-checker/issues/16). Install 18.9s cold, ~24s to a running window (SC-001 ✅). No quarantine anywhere in the bundle; `codesign --verify --strict` valid; `Signature=adhoc`; version `1.0.0`. **SC-002 observed by a human: no security dialog, no right-click-open**, via both the `PATH` command and the `/Applications` symlink — with zero `com.apple.syspolicy` and zero `CoreServicesUIAgent` log activity to corroborate. The `spctl` half is **not** obtainable and is not expected to be: it returns `rejected` on an ad-hoc signature, the recorded accepted loss, so `codesign -dv --verbose=2` is the artifact kept instead. Two findings, neither blocking `v1.0.0`: the autostart LaunchAgent pins the versioned `Cellar` path and breaks on the first `brew upgrade` ([#25](https://github.com/clintcparker/site-checker/issues/25)), and the tap reads `Untrusted` under Homebrew's new tap-trust regime.
- [ ] T045 **VALIDATE — still open, and reasoned rather than observed.** That `brew uninstall` keeps `sites.json` and that `--zap` is a silent no-op are both read out of Homebrew's source (`cmd/uninstall.rb:90-107` iterates casks only), not run. This is the task most likely to be quietly assumed done, because both claims are *almost certainly* true — which is exactly why it stays unticked until someone runs it. — quickstart Scenario 6's uninstall half (FR-003, FR-004, SC-010): add a site and quit, `brew uninstall site-checker`, confirm `~/Library/Application Support/com.clintparker.site-checker/sites.json` is **present**; re-install and confirm the sites are restored; then ~~`brew uninstall --zap site-checker` and confirm the directory is **gone**~~ → **confirm `--zap` exits 0 having changed nothing**, and that the documented `rm -rf` is what actually removes the data.
- [ ] T046 **VALIDATE — still open, and now known to be untested rather than assumed.** Every `release.yml` run is `attempt 1`; nothing has been re-run, so the update-in-place path and the skip-the-commit path have both never executed. — quickstart Scenario 7 (FR-014, SC-007): re-run the completed `v1.0.0` release from the Actions UI and confirm exactly one release for `v1.0.0` and exactly one ~~`Casks/`~~ **`Formula/`**`site-checker.rb`. Then re-run **only** the `homebrew` job and confirm it logs the unchanged-skip message, which is the path that actually exercises the skip.
- [X] T047 Confirm the post-release `verify-install-channels.yml` run (triggered by `workflow_run`) went green, including its `e2e` job, and record the result alongside the gate results in the ship record. *(Done 2026-08-11. Run `31529397548`, fired by `workflow_run` 21 seconds after the release published: `verify` green, `e2e` green. **The `schedule` trigger is a separate path and has not fired since the release** — the last five cron runs are the pre-release red streak. See the new status block.)*
